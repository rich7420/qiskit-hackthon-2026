...
from src.e009_qtsf import (  # noqa: E402
    init_weights, make_forecaster, make_state_forecaster, nmse, predict, rollout,
    temporal_qfi_diag, window_series,
)

RESULTS = ROOT / "results"
# qgr = Quantum Generative Replay: rehearse on synthetic old-task sequences generated by a frozen
# snapshot of the forecaster (memory = circuit params, not raw data). Ported here so the noise
# study can compare naive / qewc / qgr under phase-flip + bit-flip + measurement error.
METHODS = ("naive", "l2", "ewc", "qewc", "replay", "qgr")
QGR_N_SEEDS = 3      # short real seed windows kept per task to start the rollout
QGR_GEN_LEN = 48     # synthetic points generated per seed
...
def train_method(method, tasks, *, n_layers, seq_len, lr, epochs, lam, buffer_size,
                 seed, qfi_samples=16, ansatz=None, noise=None,
                 gen_len=QGR_GEN_LEN, n_gen_seeds=QGR_N_SEEDS, verbose=False):
    ansatz = ansatz or {}   # {"entangler": ..., "encoding": ...} for the gate-count ablation
    # noise = {"bit","phase","meas"} -> noisy forecaster (default.mixed) for the predictions; the
    # QFI state circuit stays noiseless (metric_tensor needs a pure state; QFI is an algorithmic
    # importance weight on the ideal geometry, not a measured quantity).
    qnode, cs, hs = make_forecaster(n_qubits=4, n_layers=n_layers, seq_len=seq_len,
                                    noise=noise, **ansatz)
    # Importance weights (QFI + head empirical Fisher) are one-off consolidation quantities;
    # compute them on the IDEAL (noiseless) model. A Jacobian on the density-matrix device is
    # ~500x costlier and would dominate runtime. Training/eval forward passes stay noisy.
    qnode_fisher = qnode if noise is None else make_forecaster(
        n_qubits=4, n_layers=n_layers, seq_len=seq_len, **ansatz)[0]
    ...
    anchors = []          # (theta_star_flat, fisher_flat) for l2/ewc/qewc
    replay_X, replay_y = [], []   # buffers for replay
    generators = []       # (circ_snapshot, head_snapshot, seed_windows) frozen quantum generators (qgr)
    ...
    for phase, task in enumerate(tasks, start=1):
        ...
        if method == "qgr" and generators:
            # Quantum Generative Replay: GENERATE synthetic old-task windows by rolling out each
            # frozen quantum generator from its seeds (under noise the generator is itself noisy).
            gx, gy = [], []
            for cw_s, hw_s, seeds in generators:
                for s in seeds:
                    ...
                    if len(Xg):
                        gx.append(Xg)
                        gy.append(yg)
            Rx = pnp.array(np.concatenate(gx), requires_grad=False) if gx else None
            Ry = pnp.array(np.concatenate(gy), requires_grad=False) if gx else None
        else:
        ...
        def cost(c, h, Xtr=Xtr, ytr=ytr, Rx=Rx, Ry=Ry):
            ...
            if method in ("replay", "qgr") and Rx is not None:
        ...
        if phase < len(tasks):   # consolidate / fill buffer for protecting this task
            ...
            if method == "l2":
                ...
                anchors.append((theta_star, _norm(empirical_fisher(qnode_fisher, cw, hw, task.X_train))))
            elif method == "qewc":
                ...
                qfi = temporal_qfi_diag(state_qnode, cw, task.X_train, n_samples=qfi_samples,
                                        seed=seed)
                head_f = empirical_fisher(qnode_fisher, cw, hw, task.X_train)[n_circ:]
                ...
                # Freeze a copy of the forecaster as this task's generator + a few real seed windows.
